...
`Wysl-媒体拆分` 是源版 `MiniMax H3 Easy 媒体拆分` 的工具仓库版本。它支持连接源版媒体包，也支持直接连接旧版多媒体加载器的图像、音频和视频列表；
通过图像、视频、音频数量控件动态生成独立输出端口，最多分别为 27、9、9 个。缺少的媒体默认只阻断对应的空分支，不会影响其他已经连接的输出；
也可以选择输出 `None`。因此不同媒体不会混到同一个端口，列表内的顺序会保持不变。
