...
由于 `grok2api` 的 WebSocket 图片消息目前只把比例、Pro 模式和图片数量传给上游，`resolution`
不一定会改变服务端返回的原始像素；节点会在收到图片后按所选比例和 `1k/2k` 进行最终尺寸归一化。
因此输出尺寸是可预测的，例如 `16:9 + 1k` 为 `1536x864`，`16:9 + 2k` 为 `3072x1728`。
